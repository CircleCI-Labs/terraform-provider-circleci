...
| Orb promotion as a resource | It is a one-shot imperative action on a mutable dev label, not a convergent desired state — there is nothing for a plan to compare against. ([NET]: the promoted record does not even reliably get a fresh id — see `PromoteOrbVersion`'s doc comment in `internal/circleci/orb.go` — which only reinforces this; it was never really about id freshness.) The client method exists and is tested. |
