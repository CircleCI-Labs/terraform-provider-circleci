...
It is used here because there is no published way to resolve a repository name to the numeric external ID that `circleci_pipeline` and `circleci_trigger` require. See `circleci_github_app_repository` for the single-repository lookup and the full rationale.
...
- `organization_id` (String) Unique identifier (UUID) of the CircleCI organization whose GitHub App repositories are listed.
