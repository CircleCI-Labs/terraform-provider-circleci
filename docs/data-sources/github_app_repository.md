...
```terraform
# Resolve a repository name to the numeric GitHub ID that circleci_pipeline and
# circleci_trigger require. Without this, the ID has to be copied out of the
# GitHub UI by hand and pasted into the configuration as a magic number.
data "circleci_github_app_repository" "api" {
  organization_id = var.organization_id
  full_name       = "acme/api"
}

resource "circleci_pipeline" "api" {
...
resource "circleci_trigger" "api_push" {
  project_id  = var.project_id
  pipeline_id = circleci_pipeline.api.id
  name        = "push"
...
- `organization_id` (String) Unique identifier (UUID) of the CircleCI organization whose GitHub App installation is searched.
