...
// production regex (the CircleCI API, in the
// v2 API) matches the two-part slug with "/", "%2f" or "%2F" as the
...
//
// This mirrors circle.http.api.v2.organization's create- and
// get-organization-handler (the v2 API behind the v2 API, reached from
// the API through a byte-for-byte reverse proxy — see
// server/simple in the CircleCI API): both answer with
// exactly {id, name, slug, vcs_type}, matching circleci-cli's
// (github.com/CircleCI-Public/circleci-cli, MIT) internal/apiclient.OrgInfo.
...
// (vcs_type "github" or "bitbucket"): create-organization-legacy only verifies
// the caller has admin access on the VCS and syncs CircleCI's record of an
// organization that already exists there. Only vcs_type "circleci" —
// create-organization-standalone — genuinely creates a new organization. See
// organizationIsStandalone in internal/provider/organization_resource.go and
...
// "Org not found.": get-organization-handler throws the same not-found
// exception whether the lookup found nothing or the caller lacks
// user-allowed-view-org? permission, so this is anti-enumeration by design —
// the opposite of the 403-for-missing behaviour DESIGN.md notes for groups.
// Either way it is reported as an error satisfying IsNotFound.
...
//
// delete-organization-handler answers 202 Accepted. It tears down the
// organization's VCS connections and — per its own documented description —
// "will delete all projects including all build data for the organization."
// Callers must only invoke this for an organization the provider genuinely
// created rather than adopted; see OrganizationInput and
// organizationIsStandalone.
//
